...
%w{tk unixODBC gmp bc libgomp libxslt unzip binutils gcc make swig autoconf wget gcc-c++ libxml2-devel pango-devel libgcj}.each do |pkg|
  package pkg do
    action :install
  end
end
...
if platform?(%w{ redhat centos fedora suse scientific amazon })
	%w{ perl-DBI rpm-build net-snmp net-snmp-utils }.each do |pkg|
		package pkg do
			action :install
		end
	end
	...
	%w{ libdbi-perl snmp snmpd }.each do |pkg|
		package pkg do
			action :install
		end
	end
